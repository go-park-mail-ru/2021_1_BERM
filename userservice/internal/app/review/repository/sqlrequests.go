...
const (
	...
	SelectAvgScore                 = "SELECT CASE WHEN SUM(score) IS NULL THEN 0 ELSE AVG(score) END AS rating, COUNT(*) AS reviews_count FROM userservice.reviews WHERE to_user_id = $1"
)
