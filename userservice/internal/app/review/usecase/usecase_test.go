...
func TestGetAllReviewsByUserIDErr3(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	mockUserRepository := userMock.NewMockRepository(ctrl)
	mockUserRepository.EXPECT().FindUserByID(review.UserId, ctx).Times(1).Return(&models.UserInfo{}, nil)
	mockUserRepository.EXPECT().FindUserByID(review.ToUserId, ctx).Times(1).Return(&models.UserInfo{}, errors.New("err"))
...
func TestGetAllReviewsByUserIDErr4(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	ctx := context.Background()
	mockReviewRepo := reviewMock.NewMockRepository(ctrl)
	mockReviewRepo.EXPECT().GetAll(uint64(2), ctx).Times(1).Return(nil, nil)
